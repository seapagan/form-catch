...
@pytest.mark.asyncio
async def test_password_is_stored_hashed(test_app, db, mocker):
    """Ensure that the raw password is not stored in the database."""
    # disable email sending by mocking the function
    _ = mocker.patch(email_fn_to_patch)
...
@pytest.mark.asyncio
async def test_verified_user_can_login(test_app, db):
...
async def test_cant_login_with_wrong_email_or_password(test_app, db, post_body):
